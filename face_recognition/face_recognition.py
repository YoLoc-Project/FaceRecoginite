...
class FaceRecognition(object):
    ...
    def predict(self, image, threshold=None):
        ...
        r = config.INPUT_HEIGHT / image.shape[0]
        dim = (int(image.shape[1] * r), int(config.INPUT_HEIGHT))
    ...
    def __predict__(self, image, threshold=None):
        ...
        for encoding, face, box in self.face_encoding(image):
            # Check face size
            if (box[2] - box[0]) < config.MIN_FACE_PREDICT_SIZE[0] or \
                    (box[3] - box[1]) < config.MIN_FACE_PREDICT_SIZE[1]:
    ...
    def face_detection(self, image):
        ...
        for face_attributes in self.face_detector.detect_faces(image_to_detect):
            if face_attributes["confidence"] > config.FACE_CONFIDENCE:
                ...
                yield (cv2.resize(face, config.FACE_ENCODING_SIZE), (x1, y1, x2, y2))
    ...
    def fit(self, folder):
        ...
        for path in tqdm.tqdm(files):
            ...
            # Resize the image while keeping the correct aspect ratio
            r = config.INPUT_HEIGHT / image.shape[0]
            dim = (int(image.shape[1] * r), int(config.INPUT_HEIGHT))
            ...
            # Get encoding
            for encoding, face, box in self.face_encoding(resized_image):
                # Add to classifier
