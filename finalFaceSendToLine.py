...
capture_interval = 30 # every 30 sec
prev_time = time.time()
port_active = 1
port_inactive =0
while True:
    ...
    if ret:
        curr_time = time.time()
        if curr_time - prev_time >= capture_interval:  #check time
            ...
            if len(result['predictions']) > 0:
                ...
                if name in newlist:
                    ...
                    GPIO.output(RELAY,port_active) # trigger relay 
                    print("door unlock")
                    time.sleep(5)
               
                    GPIO.output(RELAY,port_inactive)
                    print("door lock")
